...
description: Use when a task keeps recurring and deserves its own skill bundle in this repository's skills/ package, or when a human asks to author a new skill from an existing doc, prompt template, or workflow.
category: meta
source:
...
installed and reused the same way every other skill in `skills/` can.
...
- Anything left as a placeholder pending official-doc verification.
