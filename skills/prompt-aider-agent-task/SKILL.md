---
name: prompt-aider-agent-task
description: Adapt the Aider Agent Task Template prompt template from prompts/aider/agent-task.md with scope, safety, and verification intact.
category: prompts
source:
...
Use this skill when a task should reuse the prompt template in `prompts/aider/agent-task.md` and the human wants the template adapted without dropping its scope, safety, verification, or final-report rules. Do not use this skill for unrelated work or as permission to broaden
...
- The source file: `prompts/aider/agent-task.md`.
...
1. Read `prompts/aider/agent-task.md` in full before adapting it.
...
- Source used: `prompts/aider/agent-task.md`.
...
Remove `skills/prompt-aider-agent-task/` from the installed harness location, or remove this
...
maintenance, reviewers should treat `skills/prompt-aider-agent-task/SKILL.md` as a contract-bearing artifact
...
- Does `prompt aider agent task` state what decision, workflow, or reusable behavior it supports?
