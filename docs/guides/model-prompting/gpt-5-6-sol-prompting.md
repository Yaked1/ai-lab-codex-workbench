...
### Dated Sol identity snapshot

The checked local and published surfaces were: **installed 0.144.0, stable
0.144.1, alpha 0.145.0-alpha.4**. Standard Chat exposes a distinct **Sol Pro
path** on eligible plans. The API reasoning range is **API: none through max,
never ultra** because Ultra is an orchestration surface, not an API reasoning
value. A Business-workspace observation does not establish **universal Business
Work Ultra eligibility**. The cited **Vendor launch chart: Agents Last Exam
52.7** is a dated vendor result, not an independent reproduction.

Dated evidence labels retained for search and regression checks:

- installed 0.144.0, stable 0.144.1, alpha 0.145.0-alpha.4
- Sol Pro path
- API: none through max, never ultra
- universal Business Work Ultra eligibility is not established by one workspace observation
